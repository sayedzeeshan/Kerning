...
def CalculateGlyphHeights(baseDir,dX,enableKasheeda):
    ...
    numGlyphs += ProcessRegularGlyphs(ligatureDir,dX)
    print("Number of  glyphs processed: " + str(numGlyphs))
    # regular Haroof processing
    numGlyphs += ProcessHaroofGlyphs(haroofDir,dX)
    print("Number of glyphs processed: " + str(numGlyphs))
    if enableKasheeda:
        numGlyphs += ProcessRegularGlyphs(kasheedaDir,dX)
        print("Number of  glyphs processed: " + str(numGlyphs))
        # regular Haroof processing
        numGlyphs += ProcessHaroofGlyphs(kasheedaHaroofDir,dX)
        print("Number of glyphs processed: " + str(numGlyphs))
...
def ProcessRegularGlyphs(dir,dX):
     numGlyphs = 0
...
def ProcessHaroofGlyphs(dir,dX):
    numGlyphs = 0
    for filepath in glob.iglob(dir + '**/*.png', recursive=True):
        exceptionFlag = 0
        filename = os.path.basename(filepath)
...
def ProcessSymbolGlyphs(dir,dX):
    numGlyphs = 0
    for filepath in glob.iglob(dir + '**/*.png', recursive=True):
        exceptionFlag = 0
        filename = os.path.basename(filepath)
